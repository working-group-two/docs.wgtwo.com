      ...
      <div v-for="topic in links" :key="topic.title">
        ...
        <ul class="documentation-nav__list">
          <li v-for="item in topic.items" :key="item.id">
            ...
            <span v-else><a :href="item.externalLink" target="_blank">{{item.title}}</a>↗️</span>
          </li>
        </ul>
      </div>
    </template>
    <section>
      <nav v-if="subtitles.length">
        ...
        <ul class="menu-item anchormenu">
          <li class="anchormenu__item anchormenu__item-depth-2" :key="h2.value" v-for="h2 in subtitles">
            <a class="anchormenu__link" :href="h2.anchor">
              {{ h2.value }}
            </a>
            <ul v-if="h2.children" class="anchormenu__sub-menu">
              <li v-for="h3 in h2.children" :key="h3.value" class="anchormenu__item anchormenu__item-depth-3">
                <a class="anchormenu__link" :href="h3.anchor">
                  {{ h3.value }}
                </a>
              </li>
            </ul>
